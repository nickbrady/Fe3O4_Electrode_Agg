...
module user_input
SAVE    
     parameter(N=4,NJ=202,N_c=5,NJ_c=52,Numbertimesteps=3.6d3*500,tmax=3.6d3*500)
     ! 3.6d3 = 1 hour
     ...
     real             :: PI=3.141592654                           ! Geometric constant
          ...
          ! *********************** ELECTROLYTE  ***********************
     double precision :: eps_tot = 0.63, eps_elect = 0.47, diff = 1.00d-6 * 0.8              ! electrode porosity, diffusion coefficient
     ...
     double precision, dimension(NJ,NJ_c)     :: diff_agg_change
...
!     m_exp  = 2.0/3.0
    ...
    k_beta = 1.0d-4 * 70.0
    gamma  = 4.0
    c_alpha_sat  = 5.0 * molar_volume_Fe3O4
...
! 19    format(1(A5,1X), 1(ES15.5,1X), 2(F12.5,1X), 20(ES15.5,1X))                                         
      ...
      do l = 1, NJ
        ...
        if (it.EQ.1) then
          ! create NJ files
          write(filename, '("Time_Voltage_Position_Agg_", I3.3, ".txt")') l
          open(filenumber, file = filename, status = 'unknown')

          write(filenumber,17)  'State', 'Time', 'Voltage', 'Solution_Pot', 'Position', 'mAhg', 'Equivalence', 'Elect_Conc', &
                        'Solid_Conc', 'Theta_beta'
          write(filenumber,17) 'CDR',   'hours', 'Volts'  ,  'Volts',      'cm'      , 'mAh/g', 'LixFe3O4'   , 'mol/L'     , &
                        'LixFe3O4', 'Fraction'
        end if

        ! in each file write the agglomerate positional information
        do j_c = 1,NJ_c
          Phi_1 = cprev_c(l,2,j_c)
          Phi_2 = cprev_c(l,3,j_c)
          c0    = cprev_c(l,1,j_c)
          cs    = cprev_c(l,4,j_c)
          theta_beta = cprev_c(l,5,j_c)

          write(filenumber,15) state,  t_write,     Phi_1,     Phi_2,    xx_c(j),     mAhg,   LixFe3O4,     c0*1000,   &
                        cs*to_electrons, theta_beta
        end do
...
xmax_c                = 1.0d-4 * 1.0 /float(2)               ! Crystal size     1 um
...
!       c_density_agg = -i_rxn_0(l)
 ...
 if (theta_beta.GE.1.1d-6) then !1.0d-6
     diff_cat_W = diff_agg_change(l,j)
     diff_an_W  = diff_agg_change(l,j)

      if (diff_agg_change(l,j).LT.diff_agg*4) then 
        diff_agg_change(l,j) = diff_agg_change(l,j) * 1.01 ! slowly increase the volume of the agglomerate diffusion coefficient
      end if
!     print*, l, j, diff_cat, theta_beta
 end if

  if (theta_beta_E.GE.1.1d-6) then !1.0d-6
     diff_cat_E = diff_agg_change(l,j+1)
     diff_an_E  = diff_agg_change(l,j+1)
 end if
...
!!! PHASE CHANGE !!!
    ...
    if (cs.LT.5.0*molar_volume_Fe3O4) then
...
!         ! Spherical
        ...
        dW_c(1,1) = A_W*(-eps_agg*diff_cat_W)
        dE_c(1,1) = A_E*(-eps_agg*diff_cat_E)
        fW_c(1,1) = 0.d0
        fE_c(1,1) = 0.d0
...
!       kappa = Fconst**2*(z_cat**2*u_cat + z_an**2*u_an)*conc
        ...
        dW_c(3,1) = A_W*(-eps_agg*Fconst*(z_cat*diff_cat_W + z_an*diff_an_W))
        dE_c(3,1) = A_E*(-eps_agg*Fconst*(z_cat*diff_cat_E + z_an*diff_an_E))
